...
void WiFiManager::setupConfigPortal() {
  ...
  server->on("/ukey", std::bind(&WiFiManager::handleEncryption_Key, this));
  server->on("/ENCRYPTION_SAVE", std::bind(&WiFiManager::handleEncryption_Key_save, this));
...
void WiFiManager::handleEncryption_Key(){
  ...
  Serial.print("Encryption Key save");
  server->sendContent_P(HTTP_FORM_ENCRYPTION_KEY);
  char parLength[2];
...
void WiFiManager::handleEncryption_Key_save() {
  DEBUG_WM(F("Encryption Key save"));
  Serial.println();
  Serial.print("Encryption Key save IN handleEncryption_Key_save");
  Serial.println();
  //SAVE/mqtt info here
  Encryption_Key = urldecode(server->arg("s").c_str());
  handleRoot();
  Serial.println();
  Serial.print("done in handleMQTTSave");
  Serial.println();
